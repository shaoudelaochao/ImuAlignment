...
CVect3 double2Cvect3(const double a[3])
{
	return CVect3(a[0], a[1], a[2]);
}
...
extern double TIMEDIFF(Time_gbt   t1, Time_gbt   t2)
{
	return difftime(t1.time, t2.time) + t1.sec - t2.sec;
}


void CIMU::UpdateCnscl(const IMUObs_t *imudata, int nSamples, double &nts)
{
	...
	if (pre_imudata.time.time>0.001)
	{
		...
		/////求时间间隔
		double dt = 0;
		CVect epochtime(nSamples, 0.0);
		for (i = 0; i < nSamples; i++)
		{
			if (i < 1)
			{
				dt = TIMEDIFF(imudata[i].time, pre_imudata.time);
				epochtime.dd[i] = dt;
			}
			else
			{
				dt = TIMEDIFF(imudata[i].time, imudata[i - 1].time);
				epochtime.dd[i] = dt;
			}
		}
		...
		{
			///上一时刻的imu数据,单子样才用到
			wm_1 = double2Cvect3(pre_imudata.gyro)*epochtime.dd[0]; vm_1 = double2Cvect3(pre_imudata.acc)*epochtime.dd[0];
			cm = 1.0 / 12 * wm_1;
			sm = 1.0 / 12 * vm_1;
		}
		...
		for (i = 0; i < nSamples - 1; i++)
		{
			cm += pcf[i] * double2Cvect3(imudata[i].gyro)*epochtime.dd[i];
			sm += pcf[i] * double2Cvect3(imudata[i].acc)*epochtime.dd[i];

			wmm += double2Cvect3(imudata[i].gyro)*epochtime.dd[i];
			vmm += double2Cvect3(imudata[i].acc)*epochtime.dd[i];
		}

		wm_1 = double2Cvect3(imudata[i].gyro)*epochtime.dd[i];
		vm_1 = double2Cvect3(imudata[i].acc)*epochtime.dd[i];

		wmm += double2Cvect3(imudata[i].gyro)*epochtime.dd[i];
		vmm += double2Cvect3(imudata[i].acc)*epochtime.dd[i];


		phim = wmm + cm* double2Cvect3(imudata[i].gyro)*     epochtime.dd[i];
		dvbm = vmm + 1.0 / 2 * wmm*vmm + (cm*double2Cvect3(imudata[i].acc)*epochtime.dd[i] + sm*double2Cvect3(imudata[i].gyro)*epochtime.dd[i]);
	...
	if (nSamples == 1) pre_imudata = imudata[nSamples];
	else pre_imudata = imudata[nSamples - 1];
...
void CSINS::UpdateSins(const IMUObs_t *imudata, int nSamples, double &nts)
{

	double nts2 = 0;

	vn.i = vn.j = vn.k = 0;
	imu.UpdateCnscl( imudata, nSamples, nts);
	if (nts < 0.0000001) return;
	...
	CVect3 vn01 = vn + an*nts2;

	///位置不更新
	/*CVect3 	pos01 = pos + eth.vn2dpos(vn01, nts2);
...
*/
	...
	/////velocity updating
	fn = qnb*fb;
	an = rv2q(-eth.wnin*nts2)*fn + eth.gcc;///////// rv2q(-eth.wnin*nts2)地球自转的角度，地球自西向东，东北天坐标系
...
#if DBG_TRACE
	char time_s[64] = "";
	imutk::CGNSSTime::Time2Str(sins_time, time_s, 3);
	dbg_trace(4, "time=%s   wib=%6.3f  %6.3f  %6.3f  \n", time_s, wib.i*kRAD2DEG, wib.j*kRAD2DEG, wib.k*kRAD2DEG);
	dbg_trace(4, "time=%s   fb=%6.3f  %6.3f  %6.3f  \n", time_s, fb.i, fb.j, fb.k);
	dbg_trace(4, "time=%s   fn=%6.3f  %6.3f  %6.3f  \n", time_s, fn.i, fn.j, fn.k);
	dbg_trace(4, "time=%s   gcc=%6.3f  %6.3f  %6.3f  \n", time_s, eth.gcc.i, eth.gcc.j, eth.gcc.k);
	dbg_trace(4, "time=%s   an=%6.3f  %6.3f  %6.3f  %lf \n", time_s, an.i, an.j, an.k, nts);
	dbg_trace(4, "time=%s   vn=%6.3f  %6.3f  %6.3f    vn1=%6.3f  %6.3f  %6.3f  \n", time_s, vn.i, vn.j, vn.k, vn1.i, vn1.j, vn1.k);
#endif
...
#if DBG_TRACE
	imutk::CGNSSTime::Time2Str(sins_time, time_s, 3);
	dbg_trace(4, "time=%s   vnL=%6.3f  %6.3f  %6.3f \n", time_s, vnL.i, vnL.j, vnL.k);
#endif	
}
